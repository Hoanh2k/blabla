            ...
            <div class="row px-xl-5">
                ...
                <div class="col-lg-6">
                    <h5 class="section-title position-relative text-uppercase mb-3"><span
                            class="">Chi tiết thanh toán</span></h5>
                    <div class="bg-light p-30 mb-5">
                            ...
                            <c:forEach items="${hoaDonChiTietList}" var="list" varStatus="i">
                                <div class="product-item" style="margin-bottom: 20px; padding: 15px; border: 1px solid #ddd; border-radius: 8px;">
                                    ...
                                    <div style="display: flex; align-items: center;">
                                        <img src="/upload/${list.giayTheThaoChiTiet.giayTheThao.image.get(0).link}" width="110px" style="border-radius: 10px; margin-right: 20px;">
                                        <div>
                                            <p style="margin: 0; color: black; font-size: 16px;"><strong>${list.giayTheThaoChiTiet.giayTheThao.tenGiayTheThao}</strong></p>
                                            <p style="margin: 0; color: black; font-size: 14px;">Size: ${list.giayTheThaoChiTiet.size.size}</p>
                                            <p style="margin: 0; color: black; font-size: 14px;">Màu sắc: ${list.giayTheThaoChiTiet.mauSac.tenMauSac}</p>
                                            <p style="margin: 0; color: black; font-size: 14px;">Số lượng mua: ${list.soLuong}</p>
                                            <p style="margin: 0; color: black; font-size: 16px;">Giá bán:
                                                <span style="color: red;">
                                <fmt:formatNumber value="${list.giayTheThaoChiTiet.giayTheThao.giaBan * (100 - saleList[i.index])/100}" pattern="#,##0"/> VNĐ
                            </span>
                                            </p>
                                        </div>
                                    </div>
                        ...
                        <div class="border-bottom pt-3 pb-2">
                            <div class="border-bottom pt-3 pb-2">
                                <div class="d-flex justify-content-between mb-3">
                                    <h4 style="margin-left: 15px;color: black">Tổng tiền hàng :</h4>
                                    <h5 style="margin-right: 50px;color: red;font-size: 18px">
                                        <fmt:formatNumber type="" value="${hoaDon.thanhTien}" pattern="#,##0"/> VNĐ

                                    </h5>
                                    <input type="hidden" name="thanhTien" id="thanhTien" value="${hoaDon.thanhTien}">
                                </div>
                                <!-- Promotion Section -->
                                <div class="d-flex justify-content-between mb-3">
                                    <h4 style="margin-left: 15px;color: black">Khuyến mại :</h4>
                                    <h5 style="margin-right: 50px;color: green">
                                        <span id="promotionAmountDisplay" style="font-size: 17px"></span>
                                        <input type="hidden" id="promotionAmount" name="promotionAmount" readonly>
                                    </h5>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <h4 style="margin-left: 15px;color: black" class="font-weight-medium">Phí vận chuyển :</h4>
                            ...
                            <div class="form-group">
                                <label for="price" style="font-size: 20px;color: black;margin-bottom: 20px;margin-left:10px;margin-top: 5px;margin-left: 10px">Thành tiền:</label>
...
<%--                    Cho khuyến mãi --%>
                    ...
                    <div class="mb-5" style="background-color: #F8F9FA;margin-bottom: 30px">
                        ...
                        <div class="payment-methods">
                            <select id="ctgg-select" style="margin-left: 15px; padding: 8px; border: 1px solid #ccc; border-radius: 4px; font-size: 16px;" onchange="updatePrice(this)">
                                    ...
                                    <c:when test="${not empty CTGG}">
                                        <c:forEach items="${CTGG}" var="list">
                                            <c:if test="${list.trangThai == 1}">
                                                <option value="${list.phanTramGiam}">${list.tenChuongTrinh} - ${list.phanTramGiam}%</option>
                                            </c:if>
                                        </c:forEach>
                                    </c:when>
                                    <c:otherwise>
                                        <option value="" disabled>Không có mã khuyễn mãi nào áp dụng cho mã hóa đơn này</option>
                                    </c:otherwise>
                                </c:choose>
                            </select>
                        </div>
                        ...
                        <div class="bg-light p-30">
                            ...
                            <div class="form-group">
                                <div class="custom-control custom-radio">
                                    ...
                                    <label class="custom-control-label" for="directcheck" style="margin-left: 15px; margin-top: 10px">
...
<script>
    ...
    document.getElementById("promotionAmount").value = promotionAmount.toFixed(0);
    var promotionPriceFormatted = promotionAmount.toLocaleString('vi-VN', {minimumFractionDigits: 0, maximumFractionDigits: 3}) + " VNĐ";
    document.getElementById("promotionAmountDisplay").innerText = promotionPriceFormatted;
    var shipInput = 0;
    document.getElementById("ship").value = shipInput.toFixed(0);

    function updatePrice(selectElement) {
        var priceInput = document.getElementById("price");
        var promotionInput = document.getElementById("promotionAmount");
        var currentPrice = parseFloat(${hoaDon.thanhTien});
        var selectedValue = parseFloat(selectElement.value);
        shipInput = parseFloat(document.getElementById("ship").value.replace(/\./g, "").trim());
        var newPrice = currentPrice - (currentPrice * selectedValue / 100) + shipInput;
        promotionAmount = (currentPrice * selectedValue / 100);

        promotionPriceFormatted = promotionAmount.toLocaleString('vi-VN', {minimumFractionDigits: 0, maximumFractionDigits: 3}) + " VNĐ";

        promotionInput.value = promotionAmount.toFixed(0);
        document.getElementById("promotionAmountDisplay").innerText = promotionPriceFormatted;

        document.getElementById("price1").value = newPrice.toFixed(0);
...
<script>
    ...
    function checkBothUnchecked() {
        if (!checkbox1.checked && !checkbox2.checked) {
            ...
            const promotionAmount = parseFloat(document.getElementById("promotionAmount").value).toFixed(0);

            var tongSoTien = currentPrice - promotionAmount + shippingCost;
...
<script>
    ...
    function updatePrice(selectElement) {
        ...
        var formattedPrice1 = promotionAmount.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' });
        formattedPrice1 = '-' + formattedPrice1.replace('₫', '') + ' VNĐ';
        document.getElementById("promotionAmountDisplay").innerText = formattedPrice1;

        promotionInput.value = promotionAmount.toFixed(2);
